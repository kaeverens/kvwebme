...
if(isset($_REQUEST['id'])){
	...
	<form action="siteoptions.php?page=users&amp;id='.$id.'" method="post">';
	echo '<input type="hidden" name="id" value="'.$id.'" />';
	if (!isset($r['extras'])) {
		$r['extras']='';
	}
// { user details
	echo '<div id="details"><table><tr><th>Name</th><td><input name="name" value="'.htmlspecialchars($r['name']).'" /></td><th>Password</th><td><input name="password" type="password" /></td>'
	...
	// { phone
	echo '<th>Phone</th><td><input name="phone" value="'.htmlspecialchars($r['phone']).'" /></td></tr>';
	// }
	// { groups
...
foreach($r['address'] as $name=>$address){
	...
	echo '<table class="address-table"><tr>
	...
	<tr>
		<th>Country</th>
		<td><input type="text" name="country-'.$name.'" value="'.$address['country'].'"/></td>
	</tr>
		<th></tr></table>';
}
