...
if(isset($_REQUEST['action']) && $_REQUEST['action']=='remind'){
...
if(!$existing_accounts && isset($_REQUEST['email']) && isset($_REQUEST['password'])){
	...
	if(!filter_var($email,FILTER_VALIDATE_EMAIL))$message='Please make sure to use a valid email address';
	else{
		dbQuery(
...
else echo 'To access the administrative features of your website, you will need to enter the username and password below and click "login".';
	...
	<div id="login-tabs" style="width:40%;margin:0 auto;">
		...
		<div id="admin-login">
	   	<form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
				<table cols="3">
			   	<tr><th colspan="1"><?php echo 'email'; ?></th><td colspan="2"><input id="email" name="email" /></td></tr>
			   	<tr><th colspan="1"><?php echo 'password'; ?></th><td colspan="2"><input type="password" name="password" /></td></tr>
				<tr><th colspan="3" align="right"><input name="action" type="submit" value="<?php echo 'login'; ?>" class="login" /></th></tr>
				</table>
	   	</form>
		</div>
		<div id="admin-reminder">
	   	<form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
				<table cols="3">
			   	<tr><th colspan="1"><?php echo 'email'; ?></th><td colspan="2"><input id="email" type="text" name="email" /></td></tr>
					<tr><th colspan="3" align="right"><input name="action" type="submit" value="<?php echo 'remind'; ?>" class="login" /></th></tr>
				</table>
				<p><?php echo 'Use this form to create a new password for yourself.'; ?></p>
	   	</form>
		</div>
	</div>
	</div>
