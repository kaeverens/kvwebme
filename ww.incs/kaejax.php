...
if (!isset($SAJAX_INCLUDED)) {
	...
	function kaejax_decode_unicode_url($str) {
		// this code taken from: http://php.net/urldecode
		$res='';
		$i=0;
		$max=strlen($str)-6;
		while ($i<=$max) {
	...
	function kaejax_handle_client_request() {
		...
		if (!isset($_POST['kaejax'])) {
			return;
		}
		$unmangled=kaejax_decode_unicode_url(
			str_replace(
				array('%2B',"\r","\n","\t"),
				array('+','\r','\n','\t'),
				$_POST['kaejax']
		...
		$res=array();
		foreach ($fs as $f) {
			$res[]=call_user_func_array($f->f, $f->v);
		}
	...
	function kaejax_export() {
		...
		$n=func_num_args();
		for ($i=0;$i<$n;$i++) {
			$kaejax_export_list[]=func_get_arg($i);
		}
	}
	function kaejax_show_javascript() {
		global $kaejax_js_has_been_shown, $kaejax_is_loaded, $kaejax_export_list;
		if (!$kaejax_js_has_been_shown&&!$kaejax_is_loaded) {
			$kaejax_js_has_been_shown=1;
		}
		$html='kaejax_create_functions("'
			.preg_replace('/\&\?.*/', '', $_SERVER['REQUEST_URI'])
			.'",["'.join('","', $kaejax_export_list).'"]);';
		$kaejax_is_loaded=1;
		echo $html;
	}
	$SAJAX_INCLUDED=1;
