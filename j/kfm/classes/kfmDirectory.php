...
class kfmDirectory extends kfmObject{
	...
	function addFile($file){
		if(!$GLOBALS['kfm']->setting('allow_file_create'))return $this->error(kfm_lang('permissionDeniedCreateFile'));
	...
	function addSubdirToDb($name){
		...
		return $GLOBALS['kfm']->db->lastInsertId(KFM_DB_PREFIX.'directories','id');
	}
	...
	function checkName($file=false){
		...
		foreach($GLOBALS['kfm']->setting('banned_folders') as $ban){
			if(($ban[0]=='/' || $ban[0]=='@')&&preg_match($ban,$file))return false;
			elseif($ban==strtolower(trim($file)))return false;
		}
		if(count($GLOBALS['kfm']->setting('allowed_folders'))){
			foreach($GLOBALS['kfm']->setting('allowed_folders') as $allow){
				if($allow[0]=='/' || $allow[0]=='@'){
	...
	function createSubdir($name){
		if(!$GLOBALS['kfm']->setting('allow_directory_create'))return $this->error(kfm_lang('permissionDeniedCreateDirectory'));
		...
		chmod($physical_address,octdec('0'.$GLOBALS['kfm']->setting('default_directory_permission')));
		return $this->addSubdirToDb($name);
	}
	function delete(){
		if(!$GLOBALS['kfm']->setting('allow_directory_delete'))return $this->error(kfm_lang('permissionDeniedDeleteDirectory'));
		$files=$this->getFiles();
		foreach($files as $f){
	...
	function getPath(){
		...
		return file_join($GLOBALS['kfm']->files_root_path, $pathTmp);
	}
	/**
	...
	function relativePath($to_user_root = false){
		...
		if($to_user_root){
			...
			$root_id = $GLOBALS['kfm']->setting('root_folder_id');
		}else{
		...
		foreach($dir_iterator as $file){
			...
			if(is_dir($this->path().$filename)&&$this->checkName($filename)){
				if(!array_key_exists($filename,$dirshash)){
					...
					$dirshash[$filename]=$GLOBALS['kfm']->db->lastInsertId(KFM_DB_PREFIX.'directories','id');
				}
	...
	function moveTo($newParent){
		...
		// { check for errors
			...
			if(!$GLOBALS['kfm']->setting('allow_directory_move'))return $this->error(kfm_lang('permissionDeniedMoveDirectory'));
		...
		// { update database and kfmDirectory object
			$GLOBALS['kfm']->db->query("update ".KFM_DB_PREFIX."directories set parent=".$newParent->id." where id=".$this->id) or die('error: '.print_r($GLOBALS['kfmdb']->errorInfo(),true));
	...
	function rename($newname){
		if(!$GLOBALS['kfm']->setting('allow_directory_edit'))return $this->error(kfm_lang('permissionDeniedEditDirectory'));
		...
		$GLOBALS['kfm']->db->query("update ".KFM_DB_PREFIX."directories set name='".sql_escape($newname)."' where id=".$this->id);
		$this->name=$newname;
		$this->cached_path=$this->path();
		$GLOBALS['kfmDirectoryInstances'][$this->id]=$this;
	}
	function setDirectoryMaxSizeImage($width=0,$height=0){
		...
		$GLOBALS['kfm']->db->exec("UPDATE ".KFM_DB_PREFIX."directories SET maxwidth=$width,maxheight=$height WHERE id=".$this->id);
	}
	/*
	 * Return the max image height for the directory if set, the default otherwise
	 */
	function maxHeight(){
		return $this->maxHeight > 0 ? $this->maxHeight : $GLOBALS['kfm']->setting('max_image_upload_height');
	}
	/*
	 * Return the max image width for the directory if set, the default otherwise
	 */
	function maxWidth(){
		return $this->maxWidth > 0 ? $this->maxWidth : $GLOBALS['kfm']->setting('max_image_upload_width');
	}
	/**
