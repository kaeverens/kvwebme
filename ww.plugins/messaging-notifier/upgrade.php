...
if($version==0){ // messaging_notifier
	dbQuery('CREATE TABLE IF NOT EXISTS `messaging_notifier` (
...
if($version==1){ // messaging_notifier_rows
	dbQuery('CREATE TABLE IF NOT EXISTS `messaging_notifier_rows` (
		`mn_id` int(11),
		`mn_type` varchar(10),
		`mn_url` text
		) ENGINE=MyISAM DEFAULT CHARSET=utf8');
...
if($version==2){ // remove messaging_notifier_rows - doesn't need to be separate
