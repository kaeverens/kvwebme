...
if($version==1){ // move from kfm to database orientated - also involves renaming files
	dbQuery(
	...
	if(count($pages)!=0){
		...
		$new_images=array();
		foreach($pages as $page){
			$vars=dbAll('select value from page_vars where page_id='
				.$page['id'].' and name="image_gallery_directory" or name="gallery-template"');
			if(!$vars)
				break;
