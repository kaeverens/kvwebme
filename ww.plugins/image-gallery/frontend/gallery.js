...
var Gallery={
	options:{ // default options for the gallery
		...
		imageHeight:350,	// main image dimensions
		imageWidth:350,
	...
	// keeps track of how far through the images array the grid display is
	position:0,
	width:0,
	// grid display - the count of how many items are displayed currently
	current:0,
	height:0,
	// used to hold the timeout for the slideshow function
	t:null,
	// holds the images associated with this gallery. populated in init()
	images:{},
	cached_gallery:null,
	...
	displayGrid:function(){ // shows the grid display using a carousel
		var file, size=this.options.thumbsize, row=0, j,
			html='<table class="images-container"><tr>';
		this.current=0;
	...
	mediaDisplay:function(file){
		...
		return '<a href="'+(file.media=='image'?file.url:file.href)+'" id="'
			+Gallery.position+'"'+popup+style+'><img src="'+file.url+',width='
			+xy[0]+',height='+xy[1]+'"/></a>';
	},
			...
			case 'video': // {
				$.getScript('/ww.plugins/image-gallery/frontend/jwplayer.js',function(){
					...
					$('.ad-image img')
						...
						.attr('num',e)
						.attr('sequence',sequence);
					$('.ad-image').append('<div id="image-gallery-video" style="display:none"></div>');
