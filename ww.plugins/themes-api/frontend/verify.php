...
/**
 * frontend/verify.php, KV-Webme Themes Repository
 *
 * verifies uploaded themes, makes sure they have:
 *
 * ./h directory
 * ./c directory
 * ./screenshot.png file
 * valid HTML 5
 * header and footer panels and a right or left panel
 * if cs directory is present, all css files must have corresponding png files
 *
 * for a more detailed explanation visit <http://kvweb.me/documentation/themes>
 *
 ...
 */
shell_exec( 'cd ' . $extract_dir . ' && unzip -o ' . $theme_dir . $id . '/' . $id . '.zip' );
 ...
 */
if( !is_dir( $files_dir ) )
	themes_api_error( 'base', $id );

if( !file_exists( $files_dir . 'screenshot.png' ) )
	themes_api_error( 'screenshot', $id );

if( !is_dir( $files_dir . 'c' ) )
	themes_api_error( 'c', $id );

if( !is_dir( $files_dir . 'h' ) )
	themes_api_error( 'h', $id );
...
if( $users_c != 0 && $users != false ){
        ...
        for( $i = 0; $i < $users_c; ++$i ){
                ...
                $body = '
                        <h3>Theme Moderation</h3>
                        <p>Hi ' . $users[ $i ][ 'name' ] . ',</p>
                        <p>A new theme named "' . $name . '" has been marked for moderation, please <a href="' . $url . '/ww.admin">log in</a> and moderate the theme.</p>
                        <p>Thanks<br/>---<br/>
                        KvWebme</p>
                ';

                send_mail( $users[ $i ][ 'email' ], 'noreply@' . $_SERVER[ 'HTTP_HOST' ], 'Theme Moderation', $body, false );
