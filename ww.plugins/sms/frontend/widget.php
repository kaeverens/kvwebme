...
if(!$vars->sms_addressbook_id){
	...
	$vars->sms_addressbook_id=preg_replace('/[^,0-9]/','',$vars->sms_addressbook_id);
	$rs=dbAll('select id,name from sms_addressbooks where id in ('.$vars->sms_addressbook_id.') order by name');
}
if(!count($rs)){
	...
	if(count($rs)>1){
		...
		foreach($rs as $r){
			$list.='<li><input type="checkbox" value="'.$r['id'].'" checked="checked" />'
		...
		$list='<input style="display:none" type="checkbox" value="'.$rs[0]['id'].'" checked="checked" />';
	}
